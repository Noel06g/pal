...
function layout(opts: {
  ...
  return `<!doctype html>
<html lang="sq">
  <body style="margin:0;background:#F8F8F8;font-family:-apple-system,'Segoe UI',Arial,Helvetica,sans-serif;color:#111111;">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#F8F8F8;padding:32px 16px;">
      <tr><td align="center">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#FFFFFF;border:1px solid #EAEAEA;overflow:hidden;">
          <tr><td style="padding:24px 28px;border-bottom:1px solid #EAEAEA;">
            <span style="font-weight:700;font-size:20px;letter-spacing:-0.5px;color:#B71C1C;">${BRAND}</span>
          </td></tr>
          <tr><td style="padding:28px;">
            <h1 style="margin:0 0 12px;font-size:20px;line-height:1.3;color:#111111;">${heading}</h1>
            <div style="font-size:15px;line-height:1.6;color:#111111;">${body}</div>
            ${
              cta
                ? `<div style="margin:24px 0 8px;">
                     <a href="${cta.href}" style="display:inline-block;background:#B71C1C;color:#ffffff;text-decoration:none;font-weight:600;font-size:15px;padding:12px 22px;">${cta.label}</a>
                   </div>
                   <p style="font-size:12px;color:#6B6B6B;margin:12px 0 0;word-break:break-all;">Ose kopjo këtë lidhje: ${cta.href}</p>`
                : ""
            }
          </td></tr>
          <tr><td style="padding:18px 28px;border-top:1px solid #EAEAEA;font-size:12px;color:#6B6B6B;">
...
function twoButtons(
  ...
  return `<div>
    <a href="${acceptHref}" style="display:inline-block;background:#B71C1C;color:#fff;text-decoration:none;font-weight:600;font-size:15px;padding:12px 22px;margin-right:8px;">${acceptLabel}</a>
    <a href="${rejectHref}" style="display:inline-block;background:#ffffff;border:1px solid #F44336;color:#F44336;text-decoration:none;font-weight:600;font-size:15px;padding:11px 21px;">${rejectLabel}</a>
  </div>`;
}
...
export async function sendIdeaInviteEmail(
  ...
  const html = layout({
    ...
    body: `<p style="margin:0 0 12px;">Përshëndetje ${escapeHtml(expertName)},</p>
           ...
           <p style="font-size:12px;color:#6B6B6B;margin:16px 0 0;">Nëse nuk e ke pritur këtë email, thjesht injoroje ose kliko “Refuzo”.</p>`,
  });
