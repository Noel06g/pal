...
export function ProposeExpertGeneralButton({ loggedIn, fieldKey }: { loggedIn: boolean; fieldKey?: string }) {
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={t.idea.proposeExpert}>
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div>
            ...
            <textarea id="gp-bio" name="bio" required rows={3} className="input resize-y" />
          </div>
          <div>
            <label className="label" htmlFor="gp-reason">
              {t.forms.nomReason}
            </label>
            <textarea id="gp-reason" name="reason" required rows={2} className="input resize-y" />
          </div>
          <div>
            <label className="label" htmlFor="gp-contact">
              {t.forms.nomContact}
            </label>
            <input id="gp-contact" name="contact" required className="input" placeholder="Email ose telefon i propozuari" />
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <input id="gp-pname" name="proposerName" required className="input" />
            </div>
            <div>
              <label className="label" htmlFor="gp-pcontact">
                {t.forms.nomProposerContact}
              </label>
              <input id="gp-pcontact" name="proposerContact" required className="input" />
            </div>
          </div>
          <div>
            ...
            <input id="gp-cv" name="cv" type="file" accept="application/pdf" className="input" />
          </div>
